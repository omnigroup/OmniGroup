...
static id _commonInit(OUIUndoButton *self)
{
    ...
    // UIPushButton, a private UIControl subclass used inside the official bar button items uses this. Sadly no public API way to get it.
    [self setTitleShadowColor:[UIColor colorWithWhite:0 alpha:0.35] forState:UIControlStateNormal];
...
{
    if (!image)
        image = [[self class] _normalBackgroundImage];
...
{
    if (!image)
        image = [[self class] _highlightedBackgroundImage];
...
- (CGSize)sizeThatFits:(CGSize)size;
{
    ...
    UIImage *background = [[self class] _normalBackgroundImage];
