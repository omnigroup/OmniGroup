...
open class DisclosableInspectorPane: OUIStackedSlicesInspectorPane {
    ...
    private static var disclosedSlicesPreference = OFPreference(forKey: "OUIInspectorDisclosedSlicesPreference", defaultValue: [:])

    public func isGroupDisclosed(_ group: DisclosableSliceGroup) -> Bool {
        guard let dict = DisclosableInspectorPane.disclosedSlicesPreference.dictionaryValue as? [String:Bool] else {
            return false
        }
    ...
    fileprivate func setGroup(_ group: DisclosableSliceGroup, disclosed: Bool) {
        ...
        if let stored = DisclosableInspectorPane.disclosedSlicesPreference.dictionaryValue as? [String:Bool] {
            dict = stored
        }
        else {
            dict = DisclosableInspectorPane.disclosedSlicesPreference.defaultObjectValue as! [String:Bool]
        }
        dict[group.identifier] = disclosed
        DisclosableInspectorPane.disclosedSlicesPreference.dictionaryValue = dict
    }

//    override open func makeAvailableSegments() -> [Any]! {
//        return super.makeAvailableSegments();
//    }
...
extension DisclosableInspectorPane : DisclosableSliceGroupDelegate {
