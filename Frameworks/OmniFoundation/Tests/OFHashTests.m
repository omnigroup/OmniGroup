...
- (void)testFIPS180_1
{
    ...
    XCTAssertTrue([expectedResults count] == 3);

    XCTAssertEqualObjects([[@"abc" dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:NO] sha1Signature], [expectedResults objectAtIndex:0]);
    XCTAssertEqualObjects([[@"abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq" dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:NO] sha1Signature], [expectedResults objectAtIndex:1]);

    oneMillionAs = [[NSMutableData alloc] initWithLength:1000000];
    memset([oneMillionAs mutableBytes], 'a', [oneMillionAs length]);
    XCTAssertEqualObjects([oneMillionAs sha1Signature], [expectedResults objectAtIndex:2]);
}
...
- (void)testGilloglyGrieu
{
    ...
    XCTAssertEqualObjects([tripletsAndSuffix(6, 149, 1, 1) sha1Signature],
                  [NSData dataWithHexString:@"A3D2982427AE39C8920CA5F499D6C2BD71EBF03C" error:NULL]);

    XCTAssertEqualObjects([tripletsAndSuffix(6, 170, 3, 2) sha1Signature],
                  [NSData dataWithHexString:@"9E92C5542237B957BA2244E8141FDB66DEC730A5" error:NULL]);

    XCTAssertEqualObjects([tripletsAndSuffix(3, 490, 1, 2) sha1Signature],
...
- (void)testGilloglyGrieu_Huge
{
    ...
    @autoreleasepool {
        XCTAssertEqualObjects([tripletsAndSuffix(6, 1431655765, 1, 1) sha1Signature],
                              [NSData dataWithHexString:@"d5e09777a94f1ea9240874c48d9fecb6b634256b" error:NULL]);
    }
    @autoreleasepool {
        XCTAssertEqualObjects([tripletsAndSuffix(3, 1431655765, 0, 1) sha1Signature],
...
- (void)testRFC1321
{
    ...
    XCTAssertEqualObjects(md5string(@"abc"),
                  @"900150983cd24fb0d6963f7d28e17f72");

    XCTAssertEqualObjects(md5string(@"message digest"),
                  @"f96b697d7cb7938d525a2f31aaf161d0");

    XCTAssertEqualObjects(md5string(@"abcdefghijklmnopqrstuvwxyz"),
                  @"c3fcd3d76192e4007dfb496cca67e13b");

    XCTAssertEqualObjects(md5string(@"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789"),
                  @"d174ab98d277d9f5a5611c2c9f419d9f");

    XCTAssertEqualObjects(md5string(@"12345678901234567890123456789012345678901234567890123456789012345678901234567890"),
                  @"57edf4a22be3c955ac49da2e2107b67a");
}

- (void)testSHA256
{
    XCTAssertEqualObjects([[NSData data] sha256Signature],
    ...
    XCTAssertEqualObjects([oneMillionBytes sha256Signature],
    ...
    XCTAssertEqualObjects([[NSData dataWithBytes:"message digest" length:14] sha256Signature],
...
- (void)testSHA256_NIST
{
    ...
    XCTAssertEqualObjects([[NSData dataWithBytes:"abc" length:3] sha256Signature],
                  [NSData dataWithHexString:@"ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad" error:NULL]);
    
    XCTAssertEqualObjects([[@"abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq" dataUsingEncoding:NSASCIIStringEncoding allowLossyConversion:NO] sha256Signature],
...
- (void)testSHA256_Huge
{
    ...
    XCTAssertEqualObjects([fourGigabits sha256Signature],
...
- (void)testByName
{
    ...
    XCTAssertEqualObjects([abc signatureWithAlgorithm:@"md5"], [NSData dataWithHexString:@"900150983cd24fb0d6963f7d28e17f72" error:NULL]);
    XCTAssertEqualObjects([abc signatureWithAlgorithm:@"MD5"], [NSData dataWithHexString:@"900150983cd24fb0d6963f7d28e17f72" error:NULL]);
    XCTAssertEqualObjects([abc signatureWithAlgorithm:@"sha1"], [NSData dataWithHexString:@"A9993E364706816ABA3E25717850C26C9CD0D89D" error:NULL]);
    XCTAssertEqualObjects([abc signatureWithAlgorithm:@"SHA1"], [NSData dataWithHexString:@"A9993E364706816ABA3E25717850C26C9CD0D89D" error:NULL]);
