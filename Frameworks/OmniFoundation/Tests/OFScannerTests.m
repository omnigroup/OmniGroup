...
- (void)testScanLineComponentsSeparatedByString;
{
    NSArray *expect = [NSArray arrayWithObjects:[NSArray arrayWithObjects:@"1", @"a", @"", nil],
        ...
        nil];
    XCTAssertEqualObjects([self componentArraysFromString:@"1,a,\n2,b,"], expect);
    
    expect = [NSArray arrayWithObject:[NSArray arrayWithObjects:@"1", @"2", @"3", nil]];
    XCTAssertEqualObjects([self componentArraysFromString:@"1,2,3"], expect);
    
    expect = [NSArray arrayWithObject:[NSArray arrayWithObjects:@"String", @"1", @"Fizzle's", @"I said, \"Hi there\", and ran away.", nil]];
    XCTAssertEqualObjects([self componentArraysFromString:@"String,1,\"Fizzle's\",\"I said, \"\"Hi there\"\", and ran away.\""], expect);

    expect = [NSArray arrayWithObject:[NSArray arrayWithObjects:@"\"", @"2", nil]];
    XCTAssertEqualObjects([self componentArraysFromString:@"\"\"\"\",2"], expect);
    
    expect = [NSArray array];
    XCTAssertEqualObjects([self componentArraysFromString:@"I said,\"\"Hi there\"\".\""], expect);  // "" w/in non-quoted field == bad syntax

    expect = [NSArray arrayWithObject:[NSArray arrayWithObjects:@"I said, \"Hi\nthere\".", @"2", nil]];
    XCTAssertEqualObjects([self componentArraysFromString:@"\"I said, \"\"Hi\nthere\"\".\",2"], expect);

    expect = [NSArray array];
    XCTAssertEqualObjects([self componentArraysFromString:@"\""], expect); // one quote does not stand alone

    expect = [NSArray arrayWithObject:[NSArray arrayWithObject:@""]];
    XCTAssertEqualObjects([self componentArraysFromString:@"\n"], expect);
    ...
    expect = [NSArray array];
    XCTAssertEqualObjects([self componentArraysFromString:@""], expect);
