...
BOOL OFIsInboxFolder(NSURL *url)
{
    ...
    return (lastPathComponent != nil && [lastPathComponent hasSuffix:OFiCloudDriveInboxFolderSuffix]);
}

BOOL OFIsInInbox(NSURL *url)
{
    // Check to see if the URL directly points to the Inbox.
    if (OFIsInboxFolder(url)) {
        return YES;
    }

    // URL does not directly point to Inbox, check if it points to a file directly in the Inbox.
    return OFIsInboxFolder([url URLByDeletingLastPathComponent]);
}
