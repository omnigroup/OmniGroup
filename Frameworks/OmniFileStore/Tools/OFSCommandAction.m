...
@implementation OFSCommandAction
{
...
- initWithSpecification:(NSString *)specification action:(void (^)(void))action;
{
    ...
    for (NSString *spec in specs) {
        OFSCommandArgument *argument = [[OFSCommandArgument alloc] initWithSpecification:spec];
...
- (NSDictionary *)parseArgumentStrings:(NSArray *)argumentStrings;
{
    ...
    for (NSUInteger argumentStringIndex = 0; argumentStringIndex < argumentStringCount; argumentStringIndex++) {
        ...
        argumentValues[argument.name] = [argument valueForString:argumentStrings[argumentStringIndex]];
    }
...
- (NSString *)argumentsUsageDescription;
{
    ...
    [_requiredArguments enumerateObjectsUsingBlock:^(OFSCommandArgument *argument, NSUInteger idx, BOOL *stop){
        [argumentDescriptions addObject:argument.usageDescription];
    }];
    [_optionalArgumentsByName enumerateKeysAndObjectsUsingBlock:^(NSString *name, OFSCommandArgument *argument, BOOL *stop){
        [argumentDescriptions addObject:argument.usageDescription];
    }];
