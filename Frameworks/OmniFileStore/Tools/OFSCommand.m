...
static void OFSCommandLogv(NSString *format, va_list args, BOOL addNewlineIfMissing)
{
...
void OFSCommandLog(NSString *format, ...)
{
    va_list args;
    va_start(args, format);
    OFSCommandLogv(format, args, NO/*add newline if missing*/);
    va_end(args);
}


@implementation OFSCommand
{
...
{
    OFSCommandAction *action = [[OFSCommandAction alloc] initWithSpecification:specification action:handleCommand];
...
- (void)usage;
{
    ...
    OFSCommandLog(@"%@ supports the following commands:\n\n", [[NSProcessInfo processInfo] processName]);
...
- (void)_addActionDescriptions:(NSMutableArray *)actionDescriptions group:(NSDictionary *)group actionStack:(NSMutableArray *)actionStack;
{
    [group enumerateKeysAndObjectsUsingBlock:^(NSString *key, id entry, BOOL *stop) {
        if ([entry isKindOfClass:[OFSCommandAction class]]) {
            OFSCommandAction *action = entry;
...
- (void)error:(NSString *)format, ...;
{
    va_list args;
    va_start(args, format);
    OFSCommandLogv(format, args, YES/*add newline if missing*/);
    va_end(args);
    exit(1);
}
