...
#import <OmniFileStore/OFSRegistrationTable.h>
...
static NSInteger OFSRegistrationTableDebug = INT_MAX; // Make sure to log if we hit a log call before this is loaded from preferences/environment

#define DEBUG_TABLE(level, format, ...) do { \
    if (OFSRegistrationTableDebug >= (level)) \
        NSLog(@"TABLE %@: " format, [self shortDescription], ## __VA_ARGS__); \
} while (0)

@implementation OFSRegistrationTable
{
    NSString *_name;
...
+ (void)initialize;
{
    ...
    OBInitializeDebugLogLevel(OFSRegistrationTableDebug);
}
