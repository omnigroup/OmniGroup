// Copyright 2008-2010 Omni Development, Inc.  All rights reserved.
//
// This software may only be used and reproduced according to the
// terms in the file OmniSourceLicense.html, which should be
// distributed with this project and can also be found at
// <http://www.omnigroup.com/developer/sourcecode/sourcelicense/>.
//
// $Id$

#import <Foundation/NSObject.h>

@class NSURLRequest, NSURLCredential, NSURLAuthenticationChallenge;
@class OFSDAVFileManager;

// We usually want these in different classes, hence two protocols; you may or many not.

@protocol OFSDAVFileManagerAuthenticationDelegate <NSObject>
- (NSURLCredential *)DAVFileManager:(OFSDAVFileManager *)manager findCredentialsForChallenge:(NSURLAuthenticationChallenge *)challenge;
@end

@protocol OFSDAVFileManagerUserAgentDelegate <NSObject>
- (NSString *)DAVFileManager:(OFSDAVFileManager *)manager userAgentForRequest:(NSURLRequest *)request;
@end
